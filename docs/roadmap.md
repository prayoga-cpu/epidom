...
**2026-08-07 update — merged into one Schedule page.** `/shifts` and `/attendance` were separate nav items/pages with no owner-vs-staff view split — a staff member with either page granted saw the full manager table, and staff roles by default couldn't reach either at all. Folded both into `/schedule`, which already had the owner/staff split (`canManage`): the manager view gained a Log & History section (`schedule-log.tsx`) merging `AttendanceRecord` events and `Shift` open/close (rendered as synthetic Cash In/Out rows) into one filterable timeline, via a new `fetchUnifiedLog()`/`mergeUnifiedLog()` (`src/lib/attendance/unified-log.ts`, the latter pure and unit-tested); the staff view (`my-schedule-list.tsx`) gained Clock In/Out and — for POS-capable roles only (Cashier, Owner/Manager) — Cash In/Out actions plus a self-service "My History". `/shifts` and `/attendance` are now redirects to `/schedule`, not deleted, so old links don't dead-end. No schema change — `Shift` and `AttendanceRecord` remain two separate tables with independent financial/audit semantics; only their UI surfaces merged.
