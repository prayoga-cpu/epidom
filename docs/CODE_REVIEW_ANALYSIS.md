  ...
  if (selectedRecipe?.id && recipesData?.recipes) {
    ...
    if (updatedRecipe) {
      ...
      const hasStockChanged = updatedRecipe.ingredients.some((ing, idx) => {
        ...
        return oldIng && Number(oldIng.material.currentStock) !== Number(ing.material.currentStock);
      });
...
function hasMaterialStockChanged(oldRecipe: Recipe | null, newRecipe: Recipe): boolean {
