...
const updateKdsSettingsSchema = z.object({
...
/**
 * PATCH /api/stores/[id]/pos/kds/settings
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = updateKdsSettingsSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const updated = await prisma.store.update({
    where: { id: storeId },
    data: { kitchenDisplayEnabled: parsed.data.kitchenDisplayEnabled },
