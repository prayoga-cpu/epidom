...
- **Give your number on the customer screen.** The customer types their WhatsApp number on the pad; the till checks whether they're already a customer. A returning customer is greeted by their first name and is attached to the sale automatically — no name or email asked for. If you've already picked a customer yourself, that choice is never overwritten.
- **New customers can leave a name and email — both optional.** A number the till doesn't know opens the **New customer** form on the cashier side with the number already in it, then fills in the name and email live as the customer types them on the customer screen's on-screen keyboard. You review it and tap **Save & attach** — nothing is saved from the customer's side. Anything you type yourself in a field is left alone.
- **The New customer form is WhatsApp-first.** The WhatsApp number comes first, and name and email are optional — a number alone is enough (the customer is named after their number until you rename them in Back Office).
- **Receipts email themselves.** When an order is paid and the customer left an email — on the customer screen, or on their customer record — their receipt is emailed to them automatically. No extra tap.
- **Is the receipt emailed? Now you can see.** The **Sale complete** screen and an order's details in **Order Queue → History** now say whether the emailed receipt went out — sent (to which address, and when), failed (and why), on its way, or not sent yet — and offer **Resend** once it has been sent.
- **Fix: History said "sent via WhatsApp" for emailed receipts.** The receipt line under an order showed the last send, whichever way it went, as "Receipt sent via WhatsApp". WhatsApp and email each have their own status now.

## [2.94.0] - 2026-09-19 · feat

One shift for the whole store, a shift history for cashiers and managers, and a way to publish your schedule as a picture.

- **One shift per store.** A shift now belongs to the store, not to whoever opened it. Open it as the owner and every cashier who signs in — on that tablet or another — sees the same running shift, and their sales count against it. The Shift page says **Started by** the person who opened it, so a cashier taking over no longer sees "No active shift". A second shift can't be opened while one is running, and if two people race to open or finish one, the loser is told it's already open or already ended instead of "try again". Anyone signed in to the till can finish a shift someone else opened. If your store already has several open shifts from before this change, the newest shows first; finish it and the next one appears. **Reports name who opened a shift, not who rang each sale or who closed it** — that's how a shared shift works for now.
- **Shift history.** Under the current shift on the Shift page: every finished shift, newest first — who ran it, how many orders, the cash counted and whether the drawer balanced. Tap one to open its full report.
- **Shifts, in the Back Office.** A new **Shifts** page, next to Schedule, is the manager's shift report. For any period, and one person if you like, it lists each shift with its opening cash, cash sales, expected and counted cash and the difference — shifts that don't balance are in red, each with a link to its full report. A **Cash log** tab lists every cash in and out with the reason it was recorded; a shift's opening float and closing count are labelled as such, not as cash in and out. Managers have the page, and anyone who could see till cash on the Schedule page keeps that view — managers with a saved page list that includes Schedule are given Shifts too.
- **Attendance is only attendance.** Log & History on the Schedule page no longer mixes till cash into the clock-in timeline. Opening cash and closing counts moved to the Shifts page.
- **Schedule image.** On the Work Schedule page, choose **Shift blocks** (the grid, as before) or **Schedule image**: upload a photo or screenshot of your schedule for the dates shown, and every staff member sees it on their My Schedule page as soon as it's uploaded. Upload again for the same dates to replace it, and add a short note for the team if you want. Your choice is remembered on that device.
- **A sale can't land on a shift that's already finished.** If a tablet hasn't noticed yet that the shift was finished on another tablet, its next sale now goes to the shift that is running, or is left unlinked if none is — it used to be filed on the finished shift, which threw off that shift's cash and made the next drawer read over.
- **The Shift page keeps your count.** A brief connection drop while you're typing the counted cash no longer replaces the page with a "couldn't load" message and wipes what you entered. Signing in as another cashier no longer briefly shows "No shift" when one is open.
- **Fix: buttons cut off at the end of a shift.** In the end-of-shift dialog the **Print** button was clipped at the left edge. The buttons now wrap instead of overflowing, **Print** is shorter, and **Copy link** is just an icon.
...
- **Flatter top bar.** The search box and its scan button, **Add filter**, the printer button and your name button in the top bar are now square and run the full height of the bar with no gap above or below, and show a pointer cursor. In the top bar, **Add filter** lost its dashed outline and is now just the label until you hover it; the **Add filter** buttons on the Order Queue and Order History filter rows are unchanged.
- **The Order Queue opens on today.** On both the **Active** and **History** tabs, the orders you see now start as **today's** — everything placed since midnight on your own clock — instead of every order there has ever been. Change it from the date box next to **Unpaid** (Yesterday, Last 7 days, All time…) and tap **Reset to today** to come back. The date box is always there, so you can always see why an older order isn't listed. The **POS** / **Online ordering** counts and the **Unpaid** count cover the same day, and the "orders unpaid" alert opens **All time** so none are missing. Everyone gets this once, whatever they had filtered before; their other filters are kept.
- **Fix: "Today" starts at your midnight.** In History, "Today" and every other date range began hours after midnight for anyone east of UTC — around 07:00 in Indonesia — so an order rung up in the small hours landed under yesterday. Days now run 00:00–23:59 on your own clock, and a "Today" left open past midnight moves on to the new day by itself.
- **Light mode is navy, not grey.** Buttons, the **Charge** button, the active menu-view switch and the **POS Cashier** / **Back Office** links were a neutral dark grey in light mode; they are now the deep navy dark mode is built on, with cream text. Text, muted text and borders take a navy tint too — muted text is easier to read — and the dim behind pop-ups is navy instead of black. Dark mode is unchanged.
- **The Active Queue switch has room.** On a narrow screen the switch on the Order Queue page no longer sits against the edge of the screen.
...
- **Fix: signed out means back to the login screen.** Opening **Your Stores** after your session had expired or ended showed "Failed to load stores — Unauthorized" with a **Try Again** button that could never work. You are now sent to the login screen instead — both when you open the page and if your session ends while it is already open. Other pages already did this.
...
- **A live bill on the right, with the order queue one tap away.** The cart is now a receipt that updates as you ring up — items, discount, sub-total, service charge and tax with their rates, total. An **Order Queue** button with a live count jumps to the queue, and a **Dine In / Take Away** switch (with guests and table) lives on the bill itself, so Save Bill and checkout inherit it instead of asking again.
- **Save Bill, Print Bill, Reprint — always in reach.** "Hold order" is now **Save Bill** (a rename: bills already held work exactly as before). **Print Bill** prints a provisional bill that says it is not a receipt. **Reprint Last** stays visible, and a **More** sheet holds the rest: Custom Item, Split Bill, Merge Bill, Discount, Coupon, Redeem Points.
- **Add a customer without a pop-up.** A row at the top of the bill expands in place: search as you type, or add someone new, or leave it empty. The chip shows their points and lifetime spend. A new **Customers** page in Back Office lists everyone with customer-since, member-since, points, life spending, orders and last visit, exports to CSV, and opens each customer's orders and points ledger (owners and managers can adjust points). Life spending is always computed from real orders, never stored, so a refund or cancellation can't leave it wrong.
- **Discounts, coupons and loyalty points (Operations plan).** A new **Promotions** tab on the Data page manages discount presets (percent or fixed), coupons (code, minimum spend, use limit, dates) and the points rules. At the till the cashier taps a preset, types a coupon or redeems points; the server re-prices every one of them and never trusts an amount sent by the tablet. A coupon's last use can't be spent twice by two tills, points are earned once when a bill is paid, and refunds and cancellations take points back proportionally (a second partial refund reverses only what the first didn't). Amounts are literal in the store's own currency.
- **Split a bill by items, or pay one bill several ways.** By items, each bill becomes its own order and the discount is shared out to the cent. Or one bill can be paid with several methods — an equal split between 2–10 people, or any mix such as cash plus card — recorded as one order with a **Split payment** method and one line per payment. The cash drawer counts only the cash part, method breakdowns list the real methods, refunds ask which payment gives the money back, and receipts print one line per payment. Orders from before this release are read exactly as they were.
- **Merge Bill.** Combine saved bills into one. The merged-away bills are cancelled, never deleted, and drop out of the reports.
- **Custom Item.** Ring up something that isn't on the menu — a description, a price, and a printer area (Kitchen, Bar or none, which skips the kitchen screen).
- **A send-receipt screen after every sale.** Instead of a small print prompt: the amount paid, the **change** in large type, an email box and a WhatsApp box to send the receipt (email receipts are new), a Print Receipt button, and New Sale. A receipt can be sent at most 10 times per order.
- **Cash quick-buttons.** The exact amount plus the round-ups a customer hands over — 50.000 / 100.000 in rupiah, 5 / 10 / 20 / 50 in euro and dollar, and sensible steps for any other currency.
- **Grid, Columns or List, and barcode scanning.** Choose how the menu is laid out (remembered per device). Products have an optional **barcode** field beside the SKU; a hardware scanner adds the item straight to the bill, and the search box matches barcodes too.
- **Less top bar, and Switch user.** The search, filters, view switch and scan button now share the status bar's row on tablets, giving the menu roughly one more row of items. The staff badge is a **Switch user** button.
- **Also fixed on the way.** Receipts on 58 mm paper no longer overflow: two lines that could run past the edge (an ordinary cashier name, the "powered by" footer) now wrap, so a few receipts print slightly differently. A held bill's discount reason is cleared when the discount is removed. History's department filter now finds custom items. The public receipt page shows what was tendered and the change. A table's status can no longer be changed from another store's order.

Deliberate limits: no SMS receipts (email and WhatsApp only); no camera scanning (hardware scanners only); custom items follow the store's tax and service-charge settings; there is no take-away fee; only saved bills can be merged; a coupon or points can't be combined with split-by-items; a split payment is recorded when the bill is completed, not tender by tender; customers, coupons, points and presets need a connection (manual discount, cash and plain checkout still work offline); a saved bill can't be split by items after it is resumed. French strings for the save/resume flow still fall back to English, as they did before.

Verified: `tsc --noEmit` clean, `vitest run` 239 files / 3114 tests passing, and a pass against the development database with the real route handlers (only sign-in faked): 37 checks covering plain, split and refunded sales through the cash drawer and reports, coupons and points including replays and partial refunds, custom items, hold/resume, merge, barcodes and currency handling, plus double-finalize, double mark-paid, double refund and double cancel each raced five times against real Postgres row locks — with no test rows left behind. Independent reviews of the order-creation and payment code found and fixed three concurrency races (double-finalize, double "Mark as Paid", merge racing a payment), a refund race that could leave the drawer's expected cash short, and an under-reversal of points on repeat partial refunds. One additive database migration (customers, loyalty ledger, discount presets, coupons, loyalty settings and per-payment tables; new columns on orders, order items and products; a "Split payment" method; the receipt-send log's phone column becomes optional). Same honest gap as before: no real-browser or iPad pass in this environment, so the touch layout, the one-row top bar at tablet widths and a real barcode scanner need a human before merging to `main`; and the Xendit webhook's points credit, the shift-close route and the email/WhatsApp senders were covered by unit tests, not driven end to end.

## [2.88.0] - 2026-09-19 · feat

The Orders page now works like a counter screen: walk-in and online orders sit on their own tabs, every order gets a call-out queue number, and the selected order's details open right beside the list.

- **POS and Online ordering tabs.** The queue opens on two tabs, each showing how many orders are open in it. The Online count turns red while anything is waiting there, so a cashier working the POS tab can see an online order arrive. GoFood, GrabFood and other channels count as Online. This replaces the hidden "Source" filter; a queue saved with the old "All" setting opens on POS.
- **Queue numbers.** Every order started at the till, saved as a held bill, or placed on the storefront now gets a number — #1, #2, #3 — that restarts each morning in the store's own timezone. The bills of a split checkout share one number. Orders from before this release, and GoFood/GrabFood email imports (which carry the platform's own code), show a dash instead. You can search the queue by number ("12" or "#12"), and the number appears in Order History too, both in the table and in an order's detail.
- **Split view, now the default.** Three columns: a status list with live counts, the orders list — order number, time, queue number, customer, table and total — and the details of the order you tap: who and where, the items, how it was paid, the charges and total, with the same Start / Mark as Paid / Resume / Cancel actions as the cards. On a phone or portrait tablet there is no room for three columns, so the status list becomes a row of chips and the details open as a sheet from the bottom. Grid, Compact and Board are still one tap away in the view switcher.
- **Order History gains Queue and Table columns.**

Verified: `tsc --noEmit` clean, `vitest run` 239 files / 3114 tests passing (new coverage for the number allocator, the POS create and hold routes' wiring, and the tabs, status list, orders list, details panel and split view), and the migration applied to the development database. One additive migration: a nullable `queueNumber` on orders and a small per-store, per-day counter table. Honest gap: no real-browser or real-iPad pass in this environment — the three-column fit at 1024px landscape, the bottom sheet and touch targets need a human look, and the storefront checkout's number allocation is covered by type-check and review rather than a route test.

## [2.87.0] - 2026-09-19 · feat

Staff can now sign in to Epidom with their own account and go straight to their store's POS, and the Staff dialog's Contact and Contract cards are now optional and collapsible.

- **Invite a staff member to sign in with their own account.** Open a team member in Staff Management, make sure they have an email, and choose "Send sign-in invite". They get a single-use link, valid for 7 days, to create their Epidom account — or, if that address already has one, to sign in with it and link it. Once claimed, the staff card shows a green "Verified account" label: it means the address was proven by actually opening the link, not merely that an email was sent. A link that hasn't been used yet shows "Sign-in invite sent"; re-sending replaces the old link, and changing the person's email or deactivating them cancels it.
- **Only the invited address can claim a link.** The emailed link can create an account for its exact address, but it can never attach an account that already exists unless you are signed in as that account. Someone signed in to a different account who opens the link is refused, and the attempt is recorded in the audit trail as a critical action. Each link works once, and everything that can change between sending and claiming — the person being deactivated, their email being edited, an account appearing in the meantime — is checked again at the moment of claiming.
- **They land in their store's POS, and their PIN still applies.** After signing in, a staff account sees its store on the Stores page (tagged "Staff", without the owner's edit and delete actions) and opens straight into the POS pages their role allows. They still pick their name and enter their PIN, exactly as on a shared tablet, and they can only ever act as themselves. Their store runs on the owner's plan; they never see billing, and no plan is created for them. Staff accounts are POS-only for now: Back Office and every owner-only page stay closed to them, so Back-Office-only roles (Admin, Finance) can't be invited yet and the invite button says why.
- **A linked staff account can do what a PIN user on the owner's tablet could — reading the store's currency and tax settings, cycling a table's status, managing reservations — but not more.** Access is default-deny: anything not explicitly allowed for staff accounts stays owner-only, and editing tables, refunds and other manager actions still need a manager PIN. Cash-in/cash-out entries are not yet available to staff accounts.
- **Contact details and Contract are optional and collapse when empty.** A new team member starts with both cards closed; open one to fill it in. An existing member's card opens only if it already has something in it. "Pay rate" is now "Contract": pick Freelance, Part-time, Full-time or Contract, and set pay per hour, a monthly salary, or a percentage of sales (capped at 100%).

Verified: `tsc --noEmit` clean, `next build` succeeds, `vitest run` 150 files / 1743 tests passing, plus a 44-check end-to-end pass over HTTP against a local server and the development database (new coverage for the claim rules — including the refusal of a mismatched account, the single-use race and the duplicate-account race — the staff access policy, the invite and roster routes, the claim page in every auth state, and the Stores page for a staff-only login). Two additive database migrations (a staff-to-account link and invite tokens; a contract type and a percentage-of-sales pay type). Same honest gap as before: no real-browser or real-inbox pass in this environment, so the emailed link, both claim paths, and a real PIN login as a linked staff member need a human before merging to `main`.

## [2.86.0] - 2026-09-19 · feat

Store ownership transfer, plus a round of fixes to how a shared device switches between the Owner and staff accounts, found by live-testing the account switcher.

- **You can now transfer a store to someone else, verified by email.** Profile has a new "Transfer store ownership" card: enter the new owner's email, confirm by typing the store's name, and they get a link that's valid for 7 days. Nothing changes until they accept it while signed in to a verified account on that exact address — if they don't have an account yet they create one first, and the link survives sign-up and email verification. Accepting moves the store to their account in a single transaction and the previous owner loses all access to it: their Owner staff row is deactivated and stripped of contact details, and every live staff PIN session on the store is ended. The store keeps behaving the same for its new owner — a store that was inheriting currency, tax and payment methods from the old business gets them copied onto itself first, and a business created for the recipient inherits the store's timezone and language. Billing stays with each account (the store runs on the new owner's plan; the transfer itself charges and refunds nothing), and storefront card payments follow the new owner's Stripe Connect account, or fail closed until they connect one. Only the real owner can start, view or cancel a transfer — not a Manager or any staff account, even on the owner's own signed-in device. The link is a bearer secret, so it is single-use, only ever travels in request bodies (never a URL the audit trail or platform logs would record), and each step is recorded as a CRITICAL audit action.
- **Switching from the Owner to a Cashier no longer leaves the Owner's page on screen.** Picking a staff member on the "Who's using this device?" screen only updated the client, so a Back Office page already rendered for the Owner (Staff Management, say) stayed visible under the Cashier's name. It now reloads so the server-side access checks run for the new person and send a Cashier to POS.
- **"Switch Account" is now one thing everywhere, and no longer costs you your PIN.** The separate "back to Owner (PIN)" shortcut is gone from both the Back Office menu and POS's "More" menu. Switch Account shows the picker without ending your session, so backing out returns you to exactly where you were — labelled "POS" or "Back Office" for the screen you came from — with no PIN re-entry. Only picking someone else, or "Log Out of Staff Session", changes who is signed in.
- **The Owner row in Staff Management is now locked down.** Its page access reads MASTER — every page, all checked and not editable — and its Role, Contact Details, Pay Rate and PIN fields are removed or explained (the PIN that matters is the Owner PIN in Profile), with a pointer to the transfer card.
- **Sign-in and sign-up honour a `?next=` deep link** (validated as a same-site path), so an invited new owner lands back on the transfer page after verifying their email; Google sign-in now respects it too.

Verified: `tsc --noEmit` clean (one pre-existing, unrelated error untouched by this work), `next build` succeeds, `vitest run` 134 files / 1559 tests passing (new coverage for the transfer service's single-use token, in-transaction ownership re-check, finance carry-over and old-owner removal; the transfer routes' guards; the accept page in every auth state; the Profile card's confirm flow; MASTER lock; and the picker's session/persistence semantics). No schema change and no migration — the pending invite lives in the existing `Verification` table. Same honest gap as before: no real-browser or real-inbox pass in this environment, so the emailed link and the sign-up-then-accept path need a human before merging to `main`.
...
- **Materials/Products pagination could still get stuck showing only the first page.** The server-rendered initial list only ever fetches one page (50 materials / 20 products), and the client seeded its `total` from that page's own length instead of the real store-wide count — so on every fresh page load, pagination computed `totalPages: 1` and simply didn't render for roughly the first 20-30 seconds, until a background refetch quietly corrected it. Explains "sometimes it works, sometimes we're blocked at 50" exactly, and the identical bug existed in Products too (latent — no store had tested it with >50 products). The server component's real count is now threaded through instead of being thrown away. Verified: calling the actual server fetch function directly against this store's data now returns `total: 94`, correctly computing 2 pages.
- **Cancelling or refunding an order didn't visibly change the Finance page — or the dashboard's own analytics tiles.** The order-status and refund mutations never invalidated any revenue-reading query. Checked against the live database first: the underlying numbers were always correct (all `NON_REVENUE_STATUSES`-filtered routes exclude cancelled orders correctly); only the screen wasn't told to refresh, and it wasn't just the Finance page's dozen `finance-*` queries — the dashboard's own analytics tiles use an entirely separate `analytics-*` key namespace (found by cross-referencing every revenue-filtering API route against its actual consumer, not by re-reading the ticket), plus the Enterprise multi-store rollup (`owner-summary`) and the Storefront editor's analytics tab (`storefront-analytics*`). All four are now invalidated together, and forced to refetch even if the tab showing them isn't the one currently open.
- **Live Operations card — confirmed correct, and now proven correct end-to-end.** The absence-exclusion fix from 2.84.0 is intact; this store had never published a schedule or recorded a single attendance/absence event, so the card could only show its all-zero empty state regardless of the fix. Reproduced the exact scenario against real data: published a shift, confirmed the staff member appeared in the late list (783 minutes late), reported an absence for them, confirmed they were correctly excluded — then cleaned up the test rows. See STATUS.md for the same steps if you want to see it in the UI yourself.
...
- **When you ask us who changed something, we can answer with a person now.** Our internal activity log named whoever made each change but never said which restaurant they were working in or how to reach them, so two cashiers who share a first name at two different outlets were impossible to tell apart. Its per-person list now shows the restaurants each one worked in, their email address and their job title beside the name, and the log can be searched by restaurant.
- **Those answers are also correct now.** Whenever we narrowed the log to one severity or one outcome, the "destructive" and "refused" counts sitting beside each person threw that filter away and used their own instead of narrowing inside it, so somebody with three actions on screen could be shown a destructive count drawn from a far wider set. Two people could also have their counts merged when an automated job and an external service happened to share a name.
...
- **Low-stock warnings now come from production and waste too, not only sales.** A recipe run or a spoilage entry could take an ingredient below its minimum in complete silence — the alert only ever fired when something was sold. All three now raise the same warning and send the same notification.
- **The checkout window now tells you when ingredients come out of stock.** With the Kitchen & Bar display on, ringing up an order doesn't move stock yet — it comes out when the order is marked delivered. With it off, stock moves as soon as payment is confirmed. The window now says which applies to your store, instead of leaving it to be guessed.
- **Menu items that aren't linked to a product are now marked.** Selling one moves no stock at all, which is a perfectly valid setup but was invisible. They now carry a "No stock link" tag explaining it, alongside the existing "From Product" tag.
- **French menu labels are readable again.** Two labels in the menu editor had no French translation and printed their internal name (`storefront.menu.fromProductBadge`) on screen instead of a word.

## [2.67.0] - 2026-08-11 · feat

- **Filter your order history to one cashier shift, from open to close.** Add the new Shift filter in Order History and pick a till session — the table narrows to everything sold between that session's open and close, and the payment-method totals recompute with it. It's a time window, not just orders rung on that till, so online and delivery orders taken while the drawer was open are counted too. A session that's still open runs up to right now and is labelled as such, so a mid-shift look doesn't read as a final total.
- **Print a proper daily report, on receipt paper or on screen.** The Export button in Order History now offers a daily report alongside the existing PDF order list. It lays out the same blocks a till report is expected to have: sales, discounts, service charge and tax down to the total; invoice count and average per invoice; a cancellation summary; splits by sale type, by guest, and by payment method; and a per-product breakdown grouped under each menu category with its own subtotal. Scope it to a shift and it also reconciles the cash drawer — opening float, expected, counted, and the difference. Print it straight to a Bluetooth thermal printer at 58mm or 80mm, or open it as a page and print from any printer, including on iPad where Bluetooth printing isn't available.
- **Every daily report has its own link.** The report opens at a stable address you can bookmark, re-open, and re-print later without setting the filters up again — the same way a customer receipt already works.
- **Count guests at checkout, and see what that's worth.** Dine-in checkout now has a guest-count stepper, carried through if you park the order and finalise it later. The daily report turns it into total guests, average guests per day and average sales per guest. Orders taken before this shipped, and takeaway orders, simply have no guest count — they're left out of that block rather than counted as one guest each, and if nobody records guests the block doesn't print at all.
- **Offer the report the moment a shift closes.** The Close Shift dialog has a "print report on close" switch — turn it on and the shift's report opens as soon as you close the drawer. It's remembered for next time. The report stays one click away either way, including mid-shift, so you can check the numbers before counting the cash.
- **Slice the whole Finance page by a till session.** The same shift picker sits on Finance. Choose a session and every figure on the page — the KPI cards, the P&L, and each tab — narrows to that window, so it reconciles exactly against the printed daily report.

## [2.66.0] - 2026-08-11 · feat

- **Your dashboard now shows the shop floor, and only the cards you can actually use.** A new Live Operations card sits at the top: who is clocked in right now and for how long, which till sessions are open with their float and order count, and anyone who was on today's roster but hasn't clocked in yet — refreshed on its own, so you can leave the dashboard open on the counter screen. Times are counted on your shop's own clock, not the browser's, so a night shift lands on the right day.
- **The Production History chart hides itself when you don't use production.** Recipe-to-batch production is off by default, and the chart was still drawing a flat zero line for everyone who never turned it on. It now appears only once production is enabled, and the row it sits in re-flows so the remaining cards fill the space instead of leaving a gap.
- **Cards you can't open no longer take up the dashboard.** Stock levels, low-stock alerts, suppliers, stock movements and the new operations card belong to the Operations plan; below that tier the dashboard used to show them all locked or empty. They're now replaced by a single upgrade card, and the underlying data is no longer loaded for accounts that can't see it — so the page opens faster too.
- **Stock and alert tables read properly on a phone.** The material name column was pinned to a fifth of the row, which left roughly sixty pixels for the name on a 375px screen. Name now takes the space it needs and the number columns hold a fixed width.
- Live Operations is visible to the owner and manager accounts only — the same rule the attendance log already follows, so a cashier or kitchen login doesn't see the whole floor's clock records.

## [2.65.0] - 2026-08-11 · feat

- **Zoom the app in and out from your account menu.** Open the menu under your name and you'll find a zoom stepper — 70%, 80%, 90%, all the way up to 150%. Zoom out to fit more of the order queue on a small counter screen, or zoom in when the kitchen display is read from across the room. It replaces reaching for the browser's own zoom, which isn't available at all on a tablet (the app locks pinch-zoom so the layout can't be dragged out of shape) or in the installed app, where there's no browser toolbar to open. The setting belongs to the device, not the account, so a shared cashier tablet keeps its zoom for whoever signs in next, and it's remembered the next time the app opens.

## [2.64.3] - 2026-08-11 · fix

- **Update notes are listed newest-first again.** On a day that shipped several releases they came out in a jumbled order, so the latest update could sit halfway down the list instead of at the top. Release dates carry no time of day, which left same-day releases with nothing to sort them by — the version number now settles it. Affects both the What's new page in your dashboard and the public changelog.

## [2.64.2] - 2026-08-11 · fix

- **Orders made up entirely of custom items can be completed again.** A custom product line such as "Men's Haircut" has no kitchen prep step, so it never appears on the kitchen screen — and on an order containing nothing else, Mark All Complete quietly did nothing and the order sat at In Production with no way to move it forward. It now advances the order directly. Orders that mix custom and regular items work as they did before.
- **A brief database hiccup no longer looks like being signed out.** If the connection to the database dropped for a moment — routine on serverless Postgres, which closes idle connections and cold-starts — the app could not look up your session and reported "Unauthorized", which mid-checkout reads as a surprise logout rather than something to retry. The lookup now retries for a moment on its own, and if it still cannot be resolved you get a "please try again" message instead of a false logout. Creating, holding, finalizing and editing an order all go through this.
- **Order status badges are filled instead of outlined.** Confirmed and Ready shared a single style and In Production was a faint outline, so the queue's most important signal was easy to miss at a glance. Each status now carries its own solid color, matching the status tiles above the queue and the color stripe down the side of each card: Confirmed blue, In Production orange, Ready green, Held grey, Cancelled red, Delivered white. They read identically in light and dark mode, across the grid, compact and board views.
