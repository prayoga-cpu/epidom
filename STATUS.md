...
## ✅ 2026-09-15 — Seven Production Feedback Tickets Triaged and Fixed (2.84.0)

Operator: *"check the feedbacks one by one create a table report with status for it, translate to english, find the root causes on each, fix it one by one then report back to me with the status of each on a table and update the status on feedback production status database at the same time."* Seven tickets, all filed 2026-09-13 by one real store owner (Heloire Samuel) testing the app in French, five `BUG` and two `FEATURE_SUGGESTION`, all on `/management`, `/dashboard`, `/production`, `/pos/orders`, `/data`, and `/schedule`.

- [x] **Materials pagination (2 tickets, same root cause).** `MaterialsSection` (`src/features/dashboard/data/materials/components/materials-section.tsx`) fetched with a hardcoded `take: 50` and had no pagination UI at all — unlike `ProductsSection`, which already had page controls. With 94 materials entered, 44 were permanently invisible with no way to reach them. Ported the exact pagination pattern from `products-section.tsx` (10/20/50/100 page size, prev/next). The same `useMaterials(storeId)` call inside Management > Edit Stock (`edit-stock.tsx`) had no filters at all, defaulting to the same 50-row cap — now explicitly requests `take: 100` (the API's ceiling), matching how the same file already fetches products.
- [x] **"Reset stock to 0" had no dedicated action.** Every stock-adjustment path (`StockAdjustmentDialog`, `BulkAdjustmentDialog`) only takes a manual +/- delta with a `.positive()` Zod floor, so reaching exactly 0 meant typing the item's exact current balance by hand — not impossible, but easy to describe as "impossible." Added a "Reset to 0" quick action to `edit-stock.tsx` that posts the correct OUT adjustment (`quantity: selectedItem.currentStock`) automatically, behind a confirmation dialog.
- [x] **"How do I delete a test order, it's in my financial report" — the report exclusion already worked, the UI didn't say so.** `NON_REVENUE_STATUSES` already excludes `CANCELLED` orders from every finance report, and Order History already has a Cancel action (including for `DELIVERED` orders) plus a status filter to hide cancelled orders from the list. No hard-delete feature was added — this app treats orders as an immutable financial ledger (see the `Order` model's audit-trail comments), and a real delete would need to reconcile `StockMovement`/refund/receipt history, which is out of proportion for what turned out to be a discoverability gap, not a functional bug. Fixed the actual gap instead: the cancel-confirmation copy (`pos.orderCard.cancelConfirmDesc`/`cancelConfirmDescDelivered`, en/id) now states explicitly that cancelling excludes the order from financial reports — and those two keys, plus `cancel`/`cancelConfirmTitle`/`cancelSuccess`, were entirely **missing from `fr.ts`** (silently falling back to English on every French store), now added.
- [x] **Dashboard "Live Operations" card: absent staff stuck in the late list.** `GET /api/stores/[id]/operations/status` built its late-roster `attended` set from on-duty + clocked-in staff only; a staff member who used the "Report absence" flow was never excluded, so they sat in the amber "Rostered, not clocked in" list with a "late by Xh Ym" counter climbing all day — while simultaneously appearing in the "absences reported today" line right next to it. Now the day's `ABSENCE` records (already queried, previously only `.count()`-ed) feed into the same `attended` exclusion set.
- [x] **Production batch details: three hardcoded English labels on mobile.** `BatchDetailsDialog`'s ingredient-consumption table's mobile-only inline labels ("Qty:", "Cost:", "Total:", shown via `sm:hidden`) were plain string literals instead of `t()` calls — invisible on desktop (where the translated grid header is used instead), so it only ever showed on a phone, matching the report's total lack of detail/screenshot. Fixed by reusing the same `management.productionHistory.quantityUsed/costPerUnit/totalCost` keys the desktop header already uses — both `en.ts` and `fr.ts` already had correct translations, so no locale changes were needed.
- [x] **Schedule: could not add a second shift on the same day.** The roster grid (`schedule-client.tsx`) already stored and rendered multiple `StaffSchedule` rows per staff/day correctly (no DB constraint blocks it), but the *add* affordance only fired when a cell had zero entries, and clicking any existing shift always edited entry `[0]` regardless of which one was clicked — so a second shift (e.g. a split 8h-10h/14h-16h) could never be added or individually edited. Added an explicit "add shift" button per cell (always available, not gated on emptiness) and made the edit dialog target the specific clicked entry by id.

### Investigation notes / things intentionally not fixed here

- **`fr.ts` is ~587 lines behind `en.ts`** (5524 vs 6111 lines) — a leftover from the period `fr` was deprecated (AGENTS.md §2, reactivated 2026-08-10). Confirmed two concrete gaps in the course of this work: the `pos.orderCard` cancel/resume/hold/markPaid/refund flow (only `cancel*` was backfilled here) and the **entire `/schedule` page** (`pages.schedule*` — zero keys exist in `fr.ts`, so every string on that page falls back to English for French stores). Both are flagged, not fixed beyond what these tickets needed — a full parity backfill is a separate, much larger task the operator should decide to schedule.
- Considered adding a real order-delete feature for ticket #4 (`/pos/orders`) instead of the copy fix; deliberately did not — see the audit-trail reasoning above.

Verified: `pnpm type-check` and `pnpm lint` clean. `pnpm test` 106 files / 1210 tests, all green (no regressions). No new tests added — none of the fixes are algorithmically novel enough to need one beyond the existing `on-duty.ts`/`roster-status.ts` suites, which already cover the exclusion-set logic pattern reused here.
...
- [x] **Nothing an admin did was ever recorded before this.** `PATCH /api/admin/users` (delete-user, reset-account, set-admin, reset-password, temp-password, set-custom-price…) returned its result and forgot it happened, and an admin who qualified only via the hardcoded email list left no DB trace of ever being privileged. Both are fixed: `requireAdminApi()` (`src/lib/auth/require-admin-api.ts`) replaces all seven duplicated `requireAdmin()` copies and now returns `grantSource`, stamped onto every row that admin touches.
- [x] **Three-layer schema** (migration `20260903124248_add_audit_trail`): `ActivityEvent` (every mutating request, no row content — the coverage floor), `ActionLog` (~17 curated actions with typed payloads and hand-written `reverse()` handlers), `EntitySnapshot`/`RestoreRun` (full graph capture + identity restore for cascade roots). `actionType` is a `String`, not a Postgres enum, because `ALTER TYPE ADD VALUE` can't run in the same transaction that inserts the new value.
- [x] **Layer 1 wired into `withApiHandler` and a new `withAdminApiHandler`** — the nine admin routes never went through the merchant wrapper at all. A route→action map (`src/lib/audit/route-map.ts`) resolves ~90 known endpoints by specificity, plus a derived fallback so an unmapped mutation still produces a row instead of a silent gap. Actor resolution checks the staff-session cookie only when a store is in scope, so a POS cashier on a shared iPad is never misattributed to the store owner.
- [x] **Layer 2 wired into real routes, not left as capture-only stubs**: all 10 `admin.user.*` branches, feedback/custom-development triage, and `stock.adjust`/`waste.delete`/`pos.order.refund`/`store.delete`. Every `REVERSIBLE` entry's `roundTrip` fixture is a required field on that union member (`catalog-types.ts`) — omitting one is a `tsc` error, which matters here because `pnpm lint` lints zero TypeScript files and `pnpm test` was not in CI before this (now is).
- [x] **Reversal engine** (`reverse.ts`): ten-step guard chain — state → double-revert → depth → age → class → ledger-model guard → downstream-consumption (refuses reverting an order inside a *closed till session*, since `Shift.expectedCash` is written once and never recomputed) → catalogue precheck. A partial-unique index on `reversesActionId` makes double-revert impossible at the database level, not just in application code.
- [x] **Restore engine, live-verified against real Postgres** (Neon `development` branch): captured a store+product, cascade-deleted it, restored it with the *same* primary keys, confirmed a `Decimal(10,2)` survived the JSON round trip byte-for-byte, and confirmed a second restore against the now-live rows correctly refuses. That run caught two real bugs — see below.
- [x] **Two Prisma-7-specific bugs the plan's own design assumed away, found only by actually running it**: (1) `pg_attribute.attname::text` cast missing on two `array_agg()` calls — Postgres's internal `name[]` type isn't deserializable by the driver adapter, crashed on the very first query. (2) `Prisma.dmmf.datamodel` in Prisma 7 carries **no** `isUnique`/`isId`/`uniqueFields` on any field — the unique-constraint guard (`guards.ts`) was rewritten off DMMF onto raw `pg_index`/`pg_attribute`, matching what `restore.ts`'s planner already did correctly. `fk-graph.ts` was built DMMF-free from the start for the same reason (also no `relationOnDelete`/`relationFromFields` in v7's runtime DMMF) — corrected `AGENTS.md`/`docs/DATABASE.md`'s stale "Prisma v6" to v7.8 while here.
- [x] **Admin UI** at `/admin/activity` — Timeline / By-actor ("sort by users") / By-entity ("sort by table") / Coverage, cursor-paginated (breaking the `take: 500` + client `useMemo` convention every other admin table uses, because this table has no natural ceiling), 30s polling. Detail sheet: preview → revert with a required ≥10-char reason, annotate, flag, legal hold.
- [x] **Profile UI**: `ActivityLogCard` — what you did / what support changed on your account, the latter never naming the acting admin, with a "This wasn't me" flag that reuses the existing feedback pipeline rather than a second escalation channel.
- [x] **Alerting, previously entirely absent**: `audit/critical.recorded` emails the team on every CRITICAL action; `audit/account.changed` emails the account holder the moment support resets their password, mints a temp password, or reactivates them — the control that actually deters abuse, not just detects it after the fact.
- [x] **Retention + GDPR**: nightly `pruneAuditTrail` (365d trail / 90d snapshot payloads, legal-hold exempt) and `shredAuditSubject` (erasure by `subjectUserIds`, GIN-indexed), the latter now fired automatically at the end of `purge-expired-accounts` — which itself gained a snapshot-before-delete, making it the automated twin of admin delete-user rather than an unaudited backdoor to the same outcome.
- [x] **The one-line "realtime status" fix**: `admin-feedback-table.tsx`'s query gained `refetchInterval: 30_000` — a status change made by another admin previously stayed invisible until the viewing admin's own edit triggered a refetch.
- [x] **Kill switch**: `AUDIT_CAPTURE`/`AUDIT_SNAPSHOTS` env vars (documented in `.env.example`), fail-open by design — an unset variable must never silently disable the trail.

**Still open, documented in `docs/AUDIT_LOG_PLAN.md` §12**: the data-residency and French-retention-clause decisions from §8 are unresolved (should be closed before Layer 3 snapshots accumulate real customer data); the store-domain catalogue covers 4 of the ~30 non-admin destructive operations the original survey found (the rest are on Layer 1 only — recorded, not yet reversible); async batched restore above 5,000 rows is schema-ready (`RestoreRun.quarantined`) but not wired to an Inngest step function; `deleteBusiness` and `seed-demo` have no snapshot capture.

Verified: `npx tsc --noEmit` clean on every file this work touched (checked by filtering per-file, since the tree also carries a large concurrent cash-reconciliation feature from another session — left entirely untouched). `pnpm test` **105 files / 1193 tests, all green** (40+ new: `route-map`, `catalog`, `audit-schemas`). One genuinely flaky jsdom test (`sidebar.test.tsx`, unrelated) needed `testTimeout` raised from Vitest's 5s default in `vitest.config.ts`, since a CI gate that fails intermittently gets ignored. Ticket for this work was never filed into production Feedback — the Neon prod credential fetch was blocked twice by the permission classifier; ready-to-run SQL sits in the session scratchpad awaiting the operator's choice of access route.

---

## ✅ 2026-09-03 — Cash on Hand in the Daily Report (2.81.0)

Operator: *"Include cash on hand in the daily report when stores open. This means calculating the expected cash balance in the register at the end of the day by category (tips, petty cash, cash payments), minus any refunds, etc."*

The daily report already printed what was sold. It never answered the one question asked at closing time: how much money should be in the drawer. Where a figure existed at all it was `openingCash + all CASH orders on this till`, written once, at close.

### Three counting bugs that were live before this
- [x] **Unpaid cash orders were counted as money in the till.** The old filter was `paymentMethod === CASH && !NON_REVENUE_STATUSES.includes(status)`, and `NON_REVENUE_STATUSES` is only `["CANCELLED","HELD"]`. A `DELIVERED` order with `paymentStatus: PENDING` therefore counted — and the POS deliberately keeps those alive on the Active tab (`ACTIVE_POS_QUEUE_FILTER`), so they are common, not exotic. Every such order made the drawer look short by its own total.
- [x] **Refunds were never subtracted.** `Order.refundAmount` existed and the report even printed a refund line, but the drawer maths never touched it.
- [x] **A refund was charged back to the day of the sale.** Refunds are now attributed by `refundedAt`, not `orderDate`. This one matters more than it looks: partial refunds leave `paymentStatus` at `PAID` and only a *full* refund flips it to `REFUNDED`, so a `REFUNDED` order still counts as having collected cash and the refund is subtracted on its own day. Excluding the order outright — the obvious-looking fix — would have retroactively erased yesterday's sale the moment a refund was issued the next morning.

### What was built
- [x] **`CashMovement`** (`cash_movements`, migration `20260903130054_add_cash_movements`): TIP / PETTY_IN / PETTY_OUT / DROP / PAYOUT. `amount` is **always positive**; direction lives in `CASH_MOVEMENT_DIRECTION` in one place, so "how much was tipped today" is a plain `SUM` and no UI bug can write a negative tip. Append-only ledger — the balance is always derived.
- [x] **One shared computation.** `src/lib/finance/cash-drawer.ts` is pure and unit-tested (36 tests); `src/lib/services/cash-drawer.service.ts` owns the Prisma fetch. The shift-close PATCH, the report (browser + ESC/POS + JSON), the Finance cash tab and the dashboard card all call it, so no two screens can disagree. `ShiftReportCashDrawer` now `extends CashOnHandBreakdown` rather than restating its fields, so the shapes cannot drift.
- [x] **Live, not frozen.** `expectedCash` is computed on read. A mid-shift report shows the current position and is labelled *provisional* so it can't be mistaken for a signed-off Z-report.
- [x] **A store-wide day figure, which did not exist at all.** A date-ranged report previously produced *no* cash block (`cashDrawer` was populated only when a `shiftId` was passed). It now rolls up every till in the window plus the movements linked to none of them.
- [x] **Cash In / Out at the till** (`/schedule`, POS-capable roles), a `POST/GET /api/stores/[id]/cash-movements` pair plus a manager-gated `DELETE`, and real movements folded into the Schedule Log next to the synthetic shift open/close rows.

### The judgment call worth knowing about
- [x] **Unlinked cash is reported, not counted.** An `Order` with `paymentMethod: CASH` and `shiftId: null` is *either* a storefront order paid at the counter (money in the drawer) *or* a delivery/aggregator order settled with a courier or the platform (money not in the drawer) — and no field distinguishes them. Folding all of it into the expectation printed a shortage the size of the delivery cash; dropping it hid the counter cash. It is now its own line, `unlinkedCashSales` / "Cash Off Till", outside the total. That also keeps `expectedCash` and the summed till counts on the same scope, which is the only reason the variance means anything.

### Verification
- [x] `npx tsc --noEmit` — **zero errors in any file touched here.** (`src/hooks/use-push-notifications.ts` flickers in and out with another session's in-flight work; it was in the baseline before this task started and is untouched here.)
- [x] `npx vitest run` — **1198 passed, 105 files, zero failures.** New: 36 cash-drawer tests, plus additions to the thermal-printer, unified-log and cash-reconciliation suites.
- [x] `eslint .` clean. Note `pnpm lint` lints zero TypeScript files in this repo — only `tsc --noEmit` is a real gate.
- [x] All 42 new i18n keys verified to resolve **to strings in all three locales** by importing the dictionaries and resolving each dotted path, not by grepping.
- [x] fr strings in `receipt-labels.ts` kept **unaccented** (CP437 / `toPrinterAscii`); `src/locales/fr.ts` properly accented. Also filled 11 pre-existing `financeCash*` gaps in fr.ts that left the widened cash table half-English.
- [x] Every new ESC/POS line goes through `row()`, which guards 32-column overflow — never `formatCols()`, which does not.

### Reviewed, and what it found
A five-lens adversarial review raised 32 findings. **Its verification pass could not run — all 32 verifier agents died on API 529s — so the findings were hand-checked against the code instead.** Fixed from that pass: cash amounts in the Schedule Log and My History were being IDR-converted by the bare one-arg `formatPrice()` (the recurring currency bug class in this repo); the `DELETE` gate needed an explicit cross-store check because `requireManagerOrOwnerApi` treats a foreign-store staff session as *no* session and returns "allowed"; `amount` was unbounded against a `Decimal(12,2)` column; `occurredAt` accepted future dates; the mobile cash cards omitted three categories so their lines did not sum to their own Expected total; the new refund query had no supporting index (`orders_storeId_refundedAt_idx`, migration `20260903141930`); the reconciliation route fanned out 3N unbounded queries; the new Record Cash button was a 32px touch target on the primary iPad surface.

### ⚠️ Known limits, deliberately not fixed
- **Default day window is server-local midnight**, not the store's business timezone (`new Date(y, m, d)` in `shift-report.service.ts`). Pre-existing behaviour of that branch — the sales block already used it — and changing it would move the sales numbers too. Worth a separate pass with `getBusinessDateKey`/`businessLocalToUTC`.
- **Closed sessions are recomputed live**, so settling a cash order *after* its till closed can flag a drawer that balanced on the night. The alternative — reading the frozen `Shift.expectedCash` — resurrects the old buggy formula for every historical row. The close-time snapshot is still on the Shift row.
- **Two tills open simultaneously**: a refund is attributed by time window, so it lands against both drawers. Needs a refund ledger; `Order` records only a cumulative `refundAmount` and the latest timestamp.
- **A till spanning midnight**: no calendar-day window can equal drawer contents either way. The per-till figure answers that question correctly and is on the same report.
- **Card tips are out of scope** — only cash tips exist. A full tips picture needs an `Order`-level tip field.

---

## ✅ 2026-09-03 — Customer-Entered WhatsApp Number on the Display (2.80.0)

Operator: *"on the customer view screen, only give access to input the phone number, to prefill the cashier... give dialog that if you give the phone number, the online receipt to your whatsapp... and also give the number input without keyboard with country code and flag."*

- [x] **The channel learned to talk backwards.** Everything so far was cashier → display; this adds the one message that goes the other way, `{ type: "customer-phone", phone }` on the same `BroadcastChannel`. Deliberately the *only* reverse message: the display can't create an order, change a total, or write to the database. It hands over a suggestion and nothing else.
- [x] **It prefills, it never overwrites.** The number lands in a small non-persisted zustand store (`useCustomerPhone`) that the checkout dialog reads. The effect bails if the field already has a value, so a number the cashier typed is never clobbered. `receivedAt` is part of the key, so a number sent *after* the cashier cleared the field still applies, while the same one doesn't re-apply on every render. Cleared on both order-creation paths (online and the offline queue) so one customer's number can't ride onto the next order — and cleared on the display too, when the thank-you screen appears.
- [x] **Keyboard-free by construction, not by styling.** The number field is a plain read-only div, not an `<input>` — there is no `inputMode`, nothing focusable, so tapping it cannot raise the OS keyboard. Every digit comes from a 12-key pad (h-14/h-16 targets, well past the 40px floor in AGENTS.md). The one exception is the country *search* box, which is optional — the 245-country list is fully scrollable without it, and tapping through 245 rows would be the worse failure.
- [x] **Country code + flag** come from `react-phone-number-input`'s own `getCountries()`/`getCountryCallingCode()`/`flags`, already a dependency for the cashier's `PhoneInput` — no new package. The default is resolved from the store: `Store.country` is free text (it's matched elsewhere with `/indonesia/i`, see `inferMarket`), so the page reverse-looks-it-up against the locale name map **on the server** and hands the client a clean ISO-2, falling back to `"ID"` — the same default the cashier's own field already uses. Doing it server-side keeps the 245-entry name map out of the POS bundle.
- [x] **Validated before it is sent.** Confirm is disabled until `parsePhoneNumber(e164).isValid()` passes for the chosen country, so the cashier's form never receives something it would reject. Submitting sends E.164; "Remove" sends `null`.
- [x] **Not a Radix `Dialog`, on purpose** — this renders on the store's brand colour, so it's an overlay themed with the display's own `--cfd-*` variables (plus a new `--cfd-on-ink` for text sitting on an ink-filled button). `fixed`, not `absolute`: on a narrow display the themed root scrolls, and an absolute overlay would be centred inside that taller box and could sit off-screen. Custom properties inherit through the DOM rather than the containing block, so the theming survives the switch.

### Verification
- [x] `npx tsc --noEmit` — **zero errors in any file touched here.** Confirmed by filtering the output per-file, because the working tree currently carries a large in-flight audit-trail feature from another session.
- [x] New protocol tests in `customer-display.test.ts`; POS + locales suites green (42 tests).
- [x] Production build green, `/store/[storeId]/pos/display` included.
- [x] Every `t()` key used by the new component verified present in **all three** locales (including the reused `common.actions.back/close/delete`), by resolving each dotted path against its namespace rather than a loose grep.

### ⚠️ Pre-existing failures, not from this work
`npx vitest run` reports **19 failures, all in `src/__tests__/api/admin/users.test.ts`**, every one thrown from `captureRequestActivity` in `src/lib/audit/activity.ts` with `` `after` was called outside a request scope ``. `src/lib/audit/` and `src/lib/admin-api-handler.ts` are **untracked** — a separate, in-flight audit-trail feature that was wired into `api/admin/users/route.ts` partway through this session (the full suite was 1109/0 earlier in the same session). Left alone deliberately: it is someone else's active work, and "fixing" it would collide with it. The remaining 1092 pass.

---

## ✅ 2026-09-02 — Render Performance: Locale Code-Splitting + Query/Render Waste (2.79.0)

Operator: *"the page can't even rendered, help me to refactor the codebase to be lightweight and fast rendering... please put attention to make sure there's no important code/files to remove."*

Measured before changing anything, rather than refactoring on instinct. **The dominant cost was one chunk.**

### The headline: 687KB of translations shipped to everyone

- [x] `src/locales/index.ts` statically imported `en` + `fr` + `id` (228 + 224 + 235KB of source) and `i18n-provider.tsx` — a `"use client"` module — imported that record. **Proven, not assumed:** grepping the built chunks for a distinctive string from each language (`Layar pelanggan`, `Écran client`, `Customer display`) returned **the same file** for all three: a single **563KB / 177KB-gzip** chunk, the largest in the app, referenced by **126 of the built routes**. Every visitor downloaded, parsed and evaluated three languages to read one.
- [x] Fixed by code-splitting per locale. `en` stays a static import — it is the synchronous SSR baseline (the (app)/(public)/(auth) providers all start at `"en"` and only switch after hydration) and the target of `t()`'s missing-key fallback, so it must never be async. `fr`/`id` load through `loadLocale()` behind real `import()` calls (explicit per-locale branches, not a template literal, which would emit a context module containing the whole directory and defeat the point).
- [x] **Marketing is deliberately excluded** and its behaviour is byte-for-byte unchanged. Its language comes from the URL via middleware (`/pricing` is French, `/en/*` and `/id/*` prefixed — `src/lib/i18n-routing.ts`), so it must be correct in the server-rendered HTML for crawlers. New `i18n-provider-eager.tsx` seeds the registry at module scope for that route group only.
- [x] **Result, measured on a fresh production build:** three separate chunks. The dashboard now pulls `en` alone — **59KB gz instead of 177KB gz, ~370KB less JS to parse, on every page load**. The `fr` and `id` chunks are referenced by **29 routes each, all under `(marketing)`** — zero app routes. Felt most on the Android tills this runs on.
- [x] **Nothing was lost, and there is a test that proves it.** `src/locales/__tests__/locale-loading.test.ts` walks every dotted key path of all three lazily loaded dictionaries and asserts set-equality against the direct imports (>1000 keys each), plus in-flight request sharing and the immediate-resolve path.

### Query and render waste on every dashboard page

- [x] **The sidebar alert badge was the worst offender.** `AlertsPrefetch` sits in the (dashboard) layout, so `api/stores/[id]/alerts` runs on *every* dashboard page — and it did `include: { materialSuppliers: { include: { supplier: true } } }`, hydrating every column of every Material plus every joined Supplier row, to read nine fields. Replaced with an explicit `select`.
- [x] **Deliberately did NOT take the audit's suggested deeper fix here**, and the adversarial pass confirmed that was right: three independent verifiers flagged that porting `fetchAlertsForPage`'s `$queryRaw` predicate (`minStock > 0 AND currentStock <= minStock`) into the route would **silently delete the oversold/negative-stock alert class** — the exact case `route.ts` was written to catch, since `minStock` defaults to 0 and most stores never set it. The `select` is the behaviour-preserving half: same rows, same order, far less data.
- [x] `verifyStoreOwnership` (29 call sites, on the critical path of nearly every store-scoped page) ran two independent lookups **sequentially** and was uncached. Now one `Promise.all` and `react.cache()`d, matching `getSession`'s existing pattern. Check order is unchanged, so the error a caller sees for any given input is identical.
- [x] `getActiveStaffSession` was not react-cached, so five pages ran the same staffSession lookup twice per request. One-line fix, same precedent.
- [x] The Profile page (the one screenshotted) ran three serial awaits; the two with no data dependency now run together. The owner gate stays sequential — it must be able to redirect before anything is fetched.
- [x] `CurrencyProvider` passed a **raw object literal** as its context value, with `formatPrice`/`convertPrice`/`convertToBase`/`fetchExchangeRate` rebuilt as fresh closures each render. It wraps the entire signed-in app, so all 58 consumers re-rendered on each of its several renders per page. Now `useMemo` + `useCallback`, with the pure `round2` hoisted to module scope. `OfflineSyncProvider` had the identical defect at the root of PageShell; same fix.

### Method, and what was left alone

Ran a 5-dimension audit workflow (bundle / render-blocker / db / react / dead-code) with 3 adversarial verifiers per finding, explicitly instructed that nothing may be proposed for deletion without a proven-zero-reference grep. **Deliberately left unapplied**, because verifiers found real correctness hazards in the proposed fixes: the finance/print consolidation (folding `scheduleOrders` into `rawOrders` would apply a staff filter the By-Schedule-Shift report intentionally omits, changing printed revenue whenever `?staffId=` is set), and the composite-index proposals (`@@index([storeId, status, orderDate])` is useless — `status` is filtered with `notIn`, a `<>` predicate btree cannot use as a qual, and sitting mid-composite it blocks `orderDate` from being used at all; and dropping the standalone `@@index([storeId])` would regress `order.count`). No schema migration was made: migrations auto-apply to production on deploy, so that is the operator's call.

**Not removed: anything.** Every change is additive or a narrowing of what is fetched. No file, export, component or translation key was deleted.

### Verification
- [x] `npx tsc --noEmit` clean (the lone `use-push-notifications.ts` `Uint8Array` error is pre-existing and untouched). Note `pnpm lint` is not a gate — it lints zero TypeScript files.
- [x] **Full suite: 1109 tests passing, 0 failing.**
- [x] Production build green, run as `npx next build` — never `pnpm build`, which is `prisma migrate deploy && sync-changelog && next build` and would have hit the database.

---

## ✅ 2026-09-02 — Customer-Facing Display on a Second Screen (2.78.0)

Operator: *"on the cashier page, create an extend screen page for customer's side view what is their ordering preview only, with store/epidom's logo just as like the reference"*, plus a follow-up: *"make it optional (can be active or off) just as like the printer toggle"*.

Built from the supplied reference photo of a counter-mounted customer display: solid brand-coloured ground, a hero card for the item just rung up (name, `1X` quantity badge, price) over a large logo panel on the left, the running receipt on the right with a filled total bar pinned at its foot, and a date/time line along the top. Read-only by construction — nothing on it is tappable except the fullscreen toggle.

- [x] **New chrome-free route** `src/app/(app)/store/[storeId]/pos/display/page.tsx`, deliberately outside the `(dashboard)` group so it doesn't inherit PageShell's sidebar/topbar — same precedent as `pos/orders/print`. Because that also puts it outside `(dashboard)/pos/layout.tsx`, it applies its own gates in order: `getSession` → `verifyStoreOwnership` → `requirePlan(storeId, "POS")` → `requireStaffPageAccess(storeId, "/pos")`. Verified against the running dev server: unauthenticated → 307 to `/login`, no route collision with `(dashboard)/pos`.
- [x] **Cross-window sync over two transports, on purpose** (`src/features/pos/lib/customer-display.ts`, `hooks/use-customer-display.ts`). `BroadcastChannel` (`epidom-customer-display:<storeId>`) is the live path; a `localStorage` snapshot mirror is the cold-start path, because BroadcastChannel has **no replay** — a display window opened halfway through an order would otherwise sit blank until the cashier's next tap. The mirror doubles as the fallback for any browser without BroadcastChannel, via `storage` events. A display also posts a `request` on mount and the cashier answers with its current snapshot. Every snapshot carries `updatedAt`, and the subscriber refuses anything older than what it holds, so a late `storage` event can't resurrect stale state over a fresher broadcast.
- [x] **"What was just added" is recovered by diffing, not by widening the cart store.** `usePosCart.addItem()` either appends a line or bumps an existing one's quantity and records neither, so `resolveHighlightLineId()` diffs consecutive snapshots. A brand-new line beats a bumped one (tapping a different item is the more recent action even when both changed at once); when nothing grew — a removal, a quantity decrease, an edited note — it keeps featuring the same line so the display doesn't jump on an unrelated edit. Chose this over adding a `lastAddedId` to the persisted zustand store, which would have changed a shape already in every cashier's localStorage.
- [x] **Off by default, per till — same shape as the printer toggle** (`hooks/use-customer-display-settings.ts`, `components/pos-customer-display-menu.tsx`). A new screen icon in the POS header (both the mobile cluster and the desktop one) opens a popover with a Switch and an "Open display window" button, disabled until the switch is on. Persisted to localStorage, not the `Store` row, because whether a customer screen is plugged in is a property of *this device*, not of the store — exactly the reasoning already applied to `epidom-pos-printer-settings`. Switching it off publishes one `off` snapshot so an already-open display goes to **standby** rather than freezing on a stale order a customer might still be reading.
- [x] **Second-screen placement is a progressive enhancement.** `openCustomerDisplay()` opens the window **synchronously** — awaiting the screen lookup first would spend the click's user activation and get the popup blocked — then nudges it onto a non-primary screen via the Window Management API (`getScreenDetails`). Absent on Safari/Firefox and deniable everywhere; on any failure it stays an ordinary named popup the operator drags across once. The display page carries its own fullscreen toggle, always visible rather than hover-revealed (AGENTS.md: touch devices have no persistent hover state).
- [x] **Branding is reused from the storefront, not duplicated onto the store** — `logoUrl` / `displayName` / `themeColor`, the same reasoning `StoreReceiptSettings` already documents. Falls back to `Store.image` and the store's own name, then to `EpidomMark`.
- [x] **New `getLuminance()` / `getContrastingInk()` in `src/lib/utils/color.ts`.** `getPremiumTheme()` clamps lightness up to **85%**, so a pale brand colour is reachable and hardcoded white text on it would be unreadable. The display picks white or near-black ink by WCAG relative luminance, and the background gradient deepens *away* from that ink (toward black for white ink, toward white for dark ink) so the foot of the screen doesn't collide with it.
- [x] **A settled order says thank you.** `markCustomerDisplayPaid()` is called in `pos-checkout-dialog.tsx` on **both** success paths — online and the offline `enqueueOrder` path — immediately before `cart.clearCart()`, since it reads `cart.total` which that call zeroes. Held in a separate, non-persisted store precisely because a "paid" flag living on the cart would be wiped by the very action it announces. The screen clears itself after 12s, and the display expires a stale `paid` snapshot on its own timer too, so closing the cashier window mid-thank-you can't leave it frozen.

### Adversarial review pass, and what it caught

Ran a 5-dimension review workflow (sync / hooks / lifecycle / layout / conventions) with 3 skeptics per finding. **The verify stage hit a session limit — 39 of 68 agents errored — so most findings came back unverified and I adjudicated them by hand against the source.** Reporting that rather than the workflow's own tally, which is not trustworthy on this run. What was real and is now fixed:

- [x] **The thank-you screen swallowed the next customer's order (the worst of them).** `showPaid` was derived from the paid timer alone and never consulted the cart, so anything rung up during the 12s window was withheld while the *previous* customer's total and order number stayed on screen — then the display jumped straight to a fully built cart when the timer expired. `showPaid` now also requires an empty cart, and a non-empty cart clears the paid hold outright.
- [x] **Toggling the display off mid-thank-you stranded the paid state.** The `!enabled` branch returned before re-arming the `clearPaid` timer, whose pending copy the prior effect run's cleanup had already cancelled — so `paidAt` never reset and re-enabling within the window resurrected the settled order. The expiry now lives in its own effect, independent of both the cart and the on/off switch.
- [x] **Viewport units ignored `--app-zoom`.** Every `dvh` in the display was written bare, but `src/lib/app-zoom.ts` applies CSS `zoom` to `<html>`, which does **not** scale viewport units — so a cashier at 125% would have had the total bar clipped off the bottom of the very screen the customer reads. All five are now `calc(<len>/var(--app-zoom,1))`, matching the established form used across `(app)`. This is exactly the device the feature targets, so it would have shipped broken.
- [x] **`fr.ts` was missing `pos.cart.serviceCharge` and `pos.cart.discount`** — a pre-existing gap (the cashier's own cart already renders both), but this feature made it *customer*-visible: a French store with a service charge would have shown the raw key string across the counter. Added.
- [x] **A closed cashier window left the order frozen on the customer's screen forever.** The subscriber's self-expiry only covered `paid`, and teardown published no farewell. Now a `pagehide` listener publishes an `off` snapshot — `pagehide` deliberately, because it fires on a real close/unload but *not* on a client-side route change, so stepping over to the order queue doesn't blank the display.
- [x] **A second POS tab with a stale toggle could blank a live display.** zustand `persist` hydrates once at store creation and never listens for later writes, so a tab opened before the toggle was flipped kept `enabled: false` and broadcast `off` over the tab that was actually driving the screen. The settings store now syncs cross-tab on `storage`.
- [x] **"Just added" could lie.** Re-enabling the display over an existing cart reset the diff baseline to `[]`, making every existing line read as newly added — my own comment claimed the opposite of what the code did. `resolveHighlightLineId` became `resolveHighlight`, returning `{ id, isNew }`; the hero card drops the eyebrow on any fallback pick (a removal, or a cart it never watched being built).
- [x] Smaller, all real: the disabled path re-serialized and re-broadcast an identical `off` snapshot on every cart tap; the logo could spill outside its own rounded card at `lg` (viewport-relative `max-h` on a `flex-1` panel — now `max-h-full` + `overflow-hidden`); the header wrapped to two lines at 375px (only the store name was constrained, not the date); `requestFullscreen` was called unguarded, so where the method is undefined the TypeError escaped the click handler instead of the `.catch()`; `tracking-[0.3em]` on the EPIDOM wordmark was dead because unlayered `.epi-display` outranks Tailwind's `@layer utilities`; and the item-count chip rendered "0 items" on an empty cart, which no single plural rule gets right across en/id/fr — it's now hidden at zero.

**Judged not worth acting on:** two POS windows both answering a display's `request` (needs two `/pos` tabs on one store; last reply wins, and both carry the same shared cart), and `window.open` on a named window reloading rather than only focusing it (harmless — the display rebuilds from the localStorage mirror on mount; the misleading comment was corrected instead).

**Not verified:** how this actually looks on a real second screen. Everything above is source-level reasoning plus a route-compile check — the visual result against the reference photo still needs an eyeball.

### Tests
- [x] New `src/features/pos/lib/__tests__/customer-display.test.ts` — 16 tests, all passing. Covers modifier flattening; the highlight diff across all nine branches (new line, several at once, quantity bump, new-beats-bumped, nothing-grew-keeps-`isNew`, highlighted-line-removed, a cart it never watched being built, empty cart, quantity *decrease* must not re-highlight) including the `isNew` flag on each; snapshot parsing tolerating a null/empty/malformed/older-shape value; and `getContrastingInk` on dark, pale, 3-digit and unparseable colours.
- [x] `pnpm type-check` clean for this change, before and after the review fixes (the single remaining error, `use-push-notifications.ts` `Uint8Array` generic, is pre-existing and untouched). `pnpm lint` exits 0 but is **not** a gate here — it lints zero TypeScript files. Route verified against the running dev server: unauthenticated `/store/<id>/pos/display` → 307 to `/login`, so it compiles and doesn't collide with `(dashboard)/pos`.

---

## ✅ 2026-08-16 — Created Supplier Order Invisible in Orders to Place, + Repo-Wide Envelope Sweep (2.77.0)

Operator: *"the order is created, but doesn't showed/saved on the order stock page"*. It **was** saved — confirmed directly against the DB (both orders PENDING, with their `expiryDate`). Purely a read-path defect, and there were **two independent causes**, either of which alone empties the panel.

- [x] **Cause 1 — the SSR seed excluded the very status the panel renders.** `management/page.tsx` seeded `fetchSupplierOrdersForPage(storeId, { status: ["PLACED","RECEIVED"] })`, but that seeds ONE React Query key (`["supplier-orders","list",storeId]`) read by TWO panels: Supplier Deliveries (PLACED/RECEIVED) and Orders to Place (PENDING). The comment claimed it "matches the client-side filtering" — it matched only the deliveries half. Measured on the operator's own store: old seed → **0 orders, 0 PENDING**; new seed → **2 orders, 2 PENDING**. CANCELLED is still excluded on purpose (nothing renders it, and it accumulates forever under the `take: 100` cap).
- [x] **Cause 2 — the queryFn returned the response envelope.** Every route answers through `createSuccessResponse` (`{ success, data, meta }`), but `useSupplierOrders`/`useSupplierOrder` did `return response.json()`, so the cache held the envelope and `data.orders` / `data.order` read `undefined`. The SSR initialData is correctly shaped, which is exactly why this hid so well: the first paint looked right and the first client fetch silently blanked it. `refetchOnMount: false` + a 10s poll meant the create-invalidation refetch was the moment it broke.
- [x] Same defect in `useUpdateSupplierOrder` — it wrote the envelope into the detail cache and mapped `updatedOrder.order` (undefined) over the list, punching a hole into the row it had just edited. And in `useSupplierOrder`, which is why `SendToSupplierDialog` opened with `order === null` and a permanently disabled Send button.
- [x] **The error path was enveloped too.** `errorData.code` / `errorData.message` are nested under `errorData.error` (`createErrorResponse`), so the `SUBSCRIPTION_FEATURE_LOCKED` check never matched — a POS-plan store got a bare "Failed to fetch supplier orders" instead of the upgrade prompt. Added `unwrapError()` alongside `unwrap()`; the three mutations now use it too instead of `error.error`, which was rendering "[object Object]".
- [x] **Not changed, deliberately:** `use-suppliers.ts` has the same 403-envelope mismatch, but its fallback (`throw new ApiClientError(error, status)`) reads `response.error.message` correctly and carries `status: 403`, so the UI still degrades to the right locked state. The only loss is the access-check caching. It is a shared file on the Data › Suppliers surface; changing its gating is a wider blast radius than this bug warrants.

### Repo-wide sweep of the same bug class (operator asked to fix all of it)

A multi-agent sweep of every client fetch against every wrapping route found **14 more sites** of the identical defect. All 14 were independently verified (`routeWraps = true` on every one) and all are fixed. New shared helper `src/lib/api/unwrap.ts` — `unwrapApiData()` / `unwrapApiError()` — replaces the ad-hoc copies. Both are deliberately **tolerant** (a bare, un-enveloped payload passes through untouched), which is what makes them safe to apply without auditing every route first, and keeps them correct for SSR `initialData`, which is never enveloped.

- [x] **Live breaks — permanently empty lists.** `/api/stores/[id]/stock-movements` wraps, and all three consumers read `data.movements` off the envelope: `use-stock-movements.ts` (Adjustment History dialog), `movements/movements-tab.tsx` (the Movements ledger — a maintained Management tab per AGENTS.md §7.3), and `dashboard/recent-movements-card.tsx`. Confirmed non-vacuous: the operator's store has 2 stock movements that should have been rendering. Also `use-import-stock.ts`, whose `onSuccess` reads `data.summary` to build the "N imported, M failed" toast.
- [x] **Cache poisoning.** `prefetch-helpers.ts` (`prefetchProducts` / `prefetchRecipes` / `prefetchSuppliers`) and the two inline supplier prefetches in `materials-section.tsx` / `add-material-dialog.tsx` write their result *straight into the key the real hook reads*. An enveloped value there blanks that hook's data until the entry goes stale — up to 10 min for the supplier picker. `prefetchMaterials` in the same file already did `return data.data`, so this was an inconsistency between siblings.
- [x] **A false comment that documented the bug as intended behaviour.** `use-production-batches.ts` complete/cancel both carried `// Complete/Cancel API returns batch directly (not wrapped)`. Both routes end `NextResponse.json(createSuccessResponse(batch))` — the comment was wrong, and the raw result was never a `ProductionBatchWithRelations`. Comment corrected in place rather than deleted, so the next reader knows it was checked.
- [x] **Type-only.** `bulkDeleteProducts` / `bulkDeleteRecipes` are typed as their payload but returned the envelope, so the "deleted N" toast read `undefined`. `createMenuItemForProduct` was graded not-a-bug (caller ignores the return value); fixed anyway since it's the same one-line shape.
- [x] **The products prefetch collision is real, and not on the tab it was written for.** `prefetchProducts` writes `["products", storeId, "list", undefined]`; `products-section.tsx` reads a *different* key (it passes `productLine: "STANDARD"` + skip/take), so the prefetch never sped up the Data tab it was added for — but `waste-form-dialog.tsx` calls `useProducts(storeId, {})`, and `normalizeFilters({})` returns `undefined`, landing on exactly the poisoned key. Hover the Products tab, then open record-waste: empty item picker, and estimated loss renders 0.00.

### A second completeness sweep found four more, outside the envelope class

- [x] **`/owner` dashboard never loaded — a doubled path, not an envelope bug.** `owner-dashboard-client.tsx` called `apiClient.get("/api/owner/summary")`, but `apiClient` is constructed with `baseURL: "/api"` (`src/lib/api/client.ts:152`), so the request went to `/api/api/owner/summary` → 404 → whole page dead. Every other `apiClient` call site in the repo passes a bare path (`/stores/…`); this was the only one with the prefix, and a repo-wide grep confirms it is now the only shape in use.
- [x] **Failed image uploads showed a generic toast.** `image-upload.tsx` and `store-form.tsx` both read `error.message` from `/api/upload`'s enveloped error. Notably their *success* paths already read `data.data.url` correctly — the envelope was understood on one branch and not the other.
- [x] **Three dead subscription branches on the export paths** (`use-suppliers` / `use-recipes` / `use-products`): `error.code === "SUBSCRIPTION_FEATURE_LOCKED"` could never match, so a locked plan got a generic export failure instead of the upgrade message. Same for `useSupplierAccessCheck`'s 403 fallback, which reported "has access" for a locked plan.
- [x] **A regression I introduced and caught mid-edit:** routing those 403 reads through `unwrapApiError` left `throw new ApiClientError(error, …)` receiving the *unwrapped* `{ code, message }`. `ApiClientError`'s constructor does `super(response.error.message)`, so that would have thrown a TypeError on every non-403 supplier failure — turning a handled error into a crash. Both call sites now pass the raw envelope (`raw`), with a comment saying why; the five sibling call sites that never got an unwrapped value were left alone.

**On the sweep's own scoring:** the first workflow reported "1/14 causes survived adversarial verification", which understates it. The verify gate conflated *"is this a real defect"* with *"does it explain the supplier-order symptom"* — my prompt told the repro agent to refute anything that didn't explain that specific symptom. Reading the individual verdicts, the refuters repeatedly wrote things like *"CONFIRMED end-to-end from quoted code; I could not refute it"* and *"the underlying defect it describes is real and unfixed"* while still returning `refuted: true`. Two more artifacts: verifiers read files I had already fixed mid-run and correctly called those claims stale, and the one genuine refutation (`prefetchSuppliers` unreachable — no caller's key normalizes to the prefetch's key) argued reachability, not correctness. Every site here was re-verified by hand against both the client file and its route before being touched.

### Tests
- [x] New `src/features/dashboard/shared/hooks/__tests__/use-supplier-orders.test.ts` (7 tests, `renderHook` + stubbed `fetch`): envelope unwrapping for list and detail, the subscription-locked 403 read through `error.error`, the created order being returned for the print link, and the server's error message surfacing. **Verified the guard actually bites** — reverting the unwrap makes 2 of them fail; the refetch test asserts on a distinct `orderNumber` in the refetched payload, because an earlier version passed against the stale initialData and caught nothing.
- [x] New `src/lib/api/__tests__/unwrap.test.ts` (12 tests) builds its fixtures with the real `createSuccessResponse` / `createErrorResponse` so it tracks the envelope's actual shape, and pins the properties the call sites depend on: bare payloads pass through, an error envelope is not unwrapped as data, a payload that legitimately owns a `data` key is not stripped (`success` is the discriminator), null/undefined are safe, and unwrapping is idempotent.

---

## ✅ 2026-08-16 — Custom Price Targets a Plan, Suspends Access, Re-bills via Stripe (2.76.0)

Operator, on the admin panel's Set Custom Price dialog: *"has option to choose which plan to custom the price… then after it's changed/updated, restrict the user access and redirect this page, and trigger button to remake the payment through stripe for this custom special price"*. Two scope calls confirmed before building: **restrict Stripe-billed accounts only** (BETA/comped accounts keep the reference-only figure — suspending an account the operator granted by hand would lock it out with nothing to pay), and **cancel the old Stripe subscription immediately** rather than leaving it to bill the superseded price against suspended access.

- [x] **Schema** (migration `20260816085644_add_custom_price_offer`): `Subscription.customPricePlan` (which tier the price buys), `customPricePendingAt` (quoted, unpaid), `customPricePrevStatus` (status to hand back if the operator withdraws).
- [x] **`setCustomPrice` now takes a plan** and splits by account type. Real Stripe account → cancels the live subscription (`prorate: false`), clears `stripeSubscriptionId`, stamps the pending marker and drops `status` to `INCOMPLETE`. `admin_`/`free_` account → unchanged reference-only behaviour, no Stripe call, no suspension.
- [x] **The suspension rides on `status`, deliberately.** Every gate in the codebase — `requirePlan`, `SubscriptionService.has*Access`/`getActivePlan`, `use-feature-access`, the public storefront order/reservation routes — already reads a non-ACTIVE subscription as FREE, so one field change restricts all of them at once instead of threading a new flag through ~15 call sites.
- [x] **`requirePlan` sends a suspended owner to `/store/<id>/billing?customPrice=pending`**, not `/pricing` — /pricing cannot sell them a negotiated price. Billing itself is gated by `requireOwnerOnly`, so there's no redirect loop.
- [x] **Payment**: `POST /api/subscriptions/custom-price/checkout` → `createCustomPriceCheckoutSession` builds an ad-hoc `price_data` line item from the **stored** amount/currency/interval (the request body carries only return URLs — a client can't price itself) and tags session + subscription metadata `customPrice: "true"`.
- [x] **Webhook honesty in both directions.** Only an event carrying that marker clears `customPricePendingAt`; meanwhile `customer.subscription.updated` and `invoice.payment_succeeded` skip their ACTIVE/plan restore for a suspended row, and `customer.subscription.deleted` no longer nulls the quote (that cancellation *is* the one `setCustomPrice` issued — the old code would have wiped the offer it just created).
- [x] **Closed the three self-service ways around the suspension**: `/api/subscriptions/sync` and `/api/subscriptions/cleanup` (both force `status: ACTIVE` from Stripe) now 409 while a quote is pending, and `activateFree` throws instead of upserting ACTIVE.
- [x] **Admin dialog**: plan picker (POS/OPERATIONS/ENTERPRISE), pre-filled from the existing quote or the account's current plan; an amber panel spelling out exactly what "Quote & Suspend" does for Stripe-billed accounts; "Withdraw & Restore Access" replaces "Clear Custom Price" while pending. User rows show the quoted plan on the Custom pill plus an "Awaiting payment" pill.
- [x] **Billing page**: quote card with plan, price and a "Pay with Stripe" button, plus a line saying what stays reachable meanwhile. Strings in `en`/`id`/`fr`.
- [x] **Refactor found on the way**: `createCheckoutSession`'s customer resolution is now `resolveStripeCustomerId`, shared with the new session. It also persists the real customer id for an `admin_` account that pays — the old path created the customer but left the row on its `admin_` stub, so the account kept reporting `isBeta: true` after paying.

Verified: `pnpm type-check` clean, `pnpm lint` 0, `pnpm test` **1061 / 96 files** before the new cases and green after (13 rewritten/added across `subscription.service.test.ts` and the Stripe webhook suite). Not exercised against live Stripe — the checkout session, the immediate cancel and the settlement webhook need a Stripe test-mode pass by the operator.

**Pre-existing, not touched:** `/api/subscriptions/activate-free` lets any authenticated user self-grant any plan (`{"plan":"ENTERPRISE"}` → ACTIVE, no payment) — a beta-era shortcut its own tests pin in place. Out of scope here; flagged for a decision.

---

## ✅ 2026-08-16 — Supplier Reorder: Real Supplier List, DLC, Printable Quote (2.75.0)

Operator, from the Stock › Reorder & Deliveries page: *"this page supposed connected to the list of suppliers"*, plus asks for a DLC (France) field and a printable order quote.

- [x] **The supplier picker was reading from the alert, not the store.** `PlaceOrderDialog` populated `<SelectItem>`s from `alert.suppliers`, so the manual "Create Order" entry point (`alert` is `null` there) rendered an empty dropdown that could never satisfy its own "Please select a supplier" rule. It now reads `useSuppliers(storeId, …)` — every supplier on file — grouped into "Suppliers for this material" (with the agreed `MaterialSupplier` price, preferred first) and "Other suppliers", so a one-off substitute supplier is still selectable without editing the material.
- [x] **`unitPrice` is resolved, not assumed**: agreed `MaterialSupplier.price` → the alert's quoted price → `Material.unitCost`. The old code read `supplier.price` off the alert and threw "Supplier not found" whenever the alert path wasn't the one in play.
- [x] **Creating an order was 400-ing before it reached the database.** `createSupplierOrderSchema.expectedDate` was `z.string().datetime({ offset: true })`, but both dialogs send an `<input type="date">` value (`2026-08-16`), which that schema rejects. Replaced with a union accepting `YYYY-MM-DD`, an ISO datetime, or a `Date`. This was live on both the single and bulk order dialogs.
- [x] **`SupplierOrderItem.expiryDate`** (migration `20260816085315_supplier_order_item_expiry_date`) — the requested DLC per line. Optional, since dry goods don't carry one. Exposed in both order dialogs (bulk applies one DLC to every selected line — one delivery, one lot).
- [x] **The DLC follows the goods.** Marking an order RECEIVED copies each line's `expiryDate` onto `Material.expirationDate`, soonest-wins — that column tracks a single next-expiry date, so a sooner-expiring batch must not be masked by an older later date. Typed once, not twice.
- [x] **`updateSupplierOrderSchema` is now wired into the PATCH route** (it was declared but never used — the handler read `body` raw). Its enum said `"ORDERED"`, a value `SupplierOrderStatus` has never had; corrected to `PENDING | PLACED | RECEIVED | CANCELLED`, which is what both existing callers already send. `tax`/`shipping` lost their `.default(0)` so an unrelated PATCH can't zero stored values.
- [x] **Printable quote.** `/store/[storeId]/management/print?orderId=` already existed but was only reachable from the deliveries table. Added a "Print order quote" button to each Orders-to-Place card and to a new post-create confirmation step in the dialog (Close / Create another / Print order quote). The document gained the store's address, phone and email (header **and** repeating footer, via a new optional `storeContactLines` prop on `PrintReportShell`), the supplier's contact person, the order status, a DLC column that only prints when a line carries one, a DLC compliance line, and a two-column signature strip.
- [x] **The quote's totals were printing raw key names.** `t("finance.subtotal") || "Subtotal"` never fell back — `t()` returns the key string when it's missing, which is truthy — so the printed document read "finance.subtotal", "finance.tax", "finance.total". Added the four keys in all three locales.
- [x] **`useCreateSupplierOrder` now returns the created order** (so the dialog can link to its quote) and reads `error.error.message` instead of `error.error`, which was surfacing "[object Object]" on failure.
- [x] `fr.ts` was missing `management.delivery.orderDate` / `expectedDate` — both were silently falling back to English on the printed quote. Added, with the rest of the new strings, in `en`/`id`/`fr`.

Verified: `pnpm type-check` clean for these files, `pnpm lint` 0, `pnpm test` **1061 / 96 files** (18 new pinning the date formats, the DLC round-trip and the corrected status enum). Print route and supplier-orders API compile clean against a running dev server.

**Unrelated WIP present in the tree:** a concurrent custom-pricing change (`customPricePlan` / `customPricePendingAt`, `src/app/api/subscriptions/custom-price/`, migration `20260816085644_add_custom_price_offer`) leaves pre-existing type errors in `subscription.service.test.ts` and `admin-dashboard.tsx`. Not touched.
